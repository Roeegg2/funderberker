...
impl Hpet {
    ...
    /// Initialize the HPET
    ///
    /// NOTE: This enables the HPET, but in order for the HPET to send interrupts we need to unmask
...
impl HpetTimer {
    fn config_delivery_mode(
        ...
        if let DeliveryMode::Fsb(fsb_info) = delivery_mode {
            ...
            // NOTE: This can also be `EdgeTriggered` but we just set it to `Level`
            config.set_int_type(TriggerMode::LevelTriggered as u8);
        }

        Ok(())

        // if we're using legacy:
        //      if id is 0 => return PIC irq
        //      if id is 8 => return RTC irq
        // else if normal mode:
        //     allocate IDT entry.
        //     try to find an IRQ that is free and that is also supported by this timer (see
        //     INT_ROUTE_CAP)
        //     MAKE SURE FSB_EN_CNF IS 0!!
        //     write INT_ROUTE_CF with the allocated IRQ
        //     sanity check: read INT_ROUTE_CF to make sure the read value matches the one written
        // else if fsb:
        //      make sure it's supported (see FSB_INT_DEL_CAP)
        //      enable it
        //
    }
    ...
    fn configure(
        ...
        match timer_mode {
            TimerMode::Periodic => {
                ...
                unsafe { self.area.write(self.comparator_reg_offset(), target_cycles) };
                unsafe { self.area.write(self.comparator_reg_offset(), cycles_delta) };
            }
            TimerMode::OneShot => {
