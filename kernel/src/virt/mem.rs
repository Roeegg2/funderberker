...
use crate::{arch::{x86_64::paging::{Entry, PageSize, PageTable}, BASIC_PAGE_SIZE}, mem::{pmm::{self, PmmAllocator}, vmm::allocate_pages, PhysAddr, VirtAddr}};
...
pub(super) fn create_guest_address_space<'a>(page_count: usize) -> (GuestPhysAddr, &'a mut PageTable) {
    ...
    for i in 0..page_count {
        ...
        let page = pmm::get().allocate(NonZero::new(1).unwrap(), NonZero::new(page_count).unwrap()).unwrap();
        unsafe {
...
pub(super) fn new_guest_page_table(addr_space_pml: &mut PageTable) -> (GuestPhysAddr, &mut PageTable) {
