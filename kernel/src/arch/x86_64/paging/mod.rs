...
use crate::{arch::BASIC_PAGE_SIZE, mem::paging::PagingManager};
...
impl Entry {
    ...
    unsafe fn map_allocate(&mut self, flags: Flags<X86_64>, page_size: PageSize<X86_64>) {
        assert!(!flags.get_present(), "Entry is already present");
...
impl PageTable {
    ...
    pub unsafe fn map_pages(
        ...
        let to_skip = next_level_index(base_addr, page_size.bottom_paging_level());
        assert!(to_skip + page_count <= ENTRIES_PER_TABLE);
        for (i, entry) in table.iter_mut().enumerate().skip(to_skip).take(page_count) {
            unsafe {
                entry.map(phys_addr + i * BASIC_PAGE_SIZE, flags, page_size);
            };
        }
...
fn map_in_entry(
    ...
    sanity_assert!(total_size % X86_64::BASIC_PAGE_SIZE.size() == 0);
