...
use utils::{
    collections::stacklist::{Node, StackList}, mem::PhysAddr, sanity_assert, sync::spinlock::{SpinLock, SpinLockable}
};
...
impl PmmAllocator for BuddyAllocator<'_> {
    ...
    fn allocate(
        ...
        let (addr, bucket_index) = self.find_bucket(alignment, zone_index)?;
    ...
    unsafe fn init_from_limine(mem_map: &[&limine::memory_map::Entry]) {
        ...
        let (new_pmm, entry_base, page_count) = BuddyAllocator::new_from_limine(mem_map, total_page_count);
        *pmm = new_pmm;
        ...
        for i in 0..page_count {
            let addr = PhysAddr(entry_base + i * BASIC_PAGE_SIZE);
...
impl BuddyAllocator<'_> {
    ...
    const fn get_buddy_addr(addr: PhysAddr, zone_index: usize) -> PhysAddr {
        sanity_assert!(addr.0 % Self::index_to_bucket_size(zone_index) == 0);
    ...
    fn push_to_zone(&mut self, buddy_addr: PhysAddr, zone_index: usize) {
        sanity_assert!(self.freelist.len() > 0);
    ...
    const fn bucket_size_to_index(bucket_size: usize) -> usize {
        sanity_assert!(bucket_size.is_power_of_two());
    ...
    pub fn new_from_limine(
        ...
        let total_buffer_size = zones_size + align_to_add + FREELIST_BUCKETS_SIZE;

        // Find a matching entry in Limine's memory map
        let entry = mem_map.iter().find(|&entry| matches!(entry.entry_type, EntryType::USABLE if entry.length as usize >= total_buffer_size && entry.base != 0x0)).expect("Cannot find a usable memory entry with enough space for the buddy allocator");

        // Create a pointer to it
        let zones_ptr = PhysAddr(entry.base as usize).add_hhdm_offset().0 as *mut StackList<PhysAddr>;

        // Initialize the zones array
        for i in 0..zones_count {
            unsafe { zones_ptr.add(i).write(StackList::new()) };
        }

        let mut ret = Self {
            zones: unsafe { from_raw_parts_mut(zones_ptr, zones_count) },
            freelist: StackList::new(),
        };

        ret.init_list(zones_ptr, zones_count);

        // Mark the memory we used as taken
        (ret, entry.base as usize, total_buffer_size.div_ceil(BASIC_PAGE_SIZE))
    }

    fn init_list(&mut self, zones_ptr: *mut StackList<PhysAddr>, max_zone_level: usize) {
